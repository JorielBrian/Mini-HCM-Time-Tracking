...
interface PunchButtonProps {
  name: string;
  action: string;
  onAttendanceUpdate: () => void; // Add this prop
}

export default function PunchButton({ name, action, onAttendanceUpdate }: PunchButtonProps) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      >
        {isLoading ? (
          <div className="flex items-center justify-center">
            <RefreshCw className="animate-spin h-5 w-5 mr-3 text-white" />
            Processing...
          </div>
        ) : (
          <div className="flex items-center justify-center">
      ...
      {message && (
        <div className={`mt-2 p-3 rounded-lg text-sm font-medium w-64 text-center transition-all duration-300 animate-in fade-in slide-in-from-top-2 ${
